  ...
  //-----------------------------------------------------------------------
    ...
    private static Date parseDateWithLeniency(
        ...
        for (final String parsePattern : parsePatterns) {
            ...
            // Make parsing more permissive for locales that use trailing dots
            // in abbreviations (e.g., German "Mi." or "Apr.") by stripping
            // trailing dots from month and weekday symbols so inputs like
            // "Mi, 09 Apr 2008 ..." parse correctly.
            try {
                final DateFormatSymbols dfs = parser.getDateFormatSymbols();
                String[] shortWeekdays = dfs.getShortWeekdays();
                for (int i = 0; i < shortWeekdays.length; i++) {
                    if (shortWeekdays[i] != null) {
                        shortWeekdays[i] = shortWeekdays[i].replaceAll("\\.$", "");
                    }
                }
                ...
                for (int i = 0; i < shortMonths.length; i++) {
                    if (shortMonths[i] != null) {
                        shortMonths[i] = shortMonths[i].replaceAll("\\.$", "");
                    }
                }
                ...
                // If anything goes wrong with symbol normalization, fall back
                // to default behavior and continue to attempt parsing.
            }
